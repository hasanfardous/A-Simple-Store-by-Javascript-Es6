...
const showProducts = (products) => {
  const searchKeyword = document.getElementById("input-field");
  searchKeyword.value = '';             // Resetting value
  const allProductsElement = document.getElementById("all-products");
  allProductsElement.innerHTML = '';    // Resetting inner contents before printing
  const allProducts = products.map((pd) => pd);
  for (const product of allProducts) {
    const image = product.image;        // This property called wrongly, I just change this
    ...
    <img class="product-image" src=${image}></img>
      ...
      <p class="ratings">${getRatingHtml(product.rating.rate)} ${product.rating.rate} | ${product.rating.count} Reviews</p>
      <h3>Price: $ ${product.price}</h3>
      <div class="product-buttons d-flex">
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-warning">Add to cart <i class="glyphicon glyphicon-shopping-cart"></i></button>
      <button id="details-btn" class="btn btn-info">Details</button></div></div>
      `;
    allProductsElement.appendChild(div);
  }
};
